...
При запуске exit node:
1. Создаётся TUN интерфейс (например, `utun5`)
2. Автоматически определяется внешний интерфейс (например, `en0`)
3. Включается IP forwarding
4. Добавляются правила NAT
5. При завершении (Ctrl+C) все настройки откатываются

### Конфигурация

В `config/exit-node.json`:
...
- `enabled` — включить автоматическую настройку NAT (по умолчанию `true`)
- `externalInterface` — внешний интерфейс (`null` = автоопределение)

---

## Ручная настройка (fallback)

Если автоматическая настройка не работает, используйте npm-скрипты:
