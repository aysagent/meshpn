...
Полный runner — `npm run transparent-tls:acceptance`; необходимые браузеры, Go и pcap-инструменты перечислены в инструкции. Это не команда запуска VPN. Последний зафиксированный полный прогон: **730 Node-тестов + 14 браузерных сценариев, PASS**, включая расширенный DNS wire contract. DNS adapter ранее также прошёл [независимый pcap и два пятиминутных soak](scripts/dns-adapter-soak.md): IPv4/transparent и IPv6/combo, обрывы, переполнение и отмены; ресурсы освобождены, память в заданных бюджетах. Отдельные 9 новых real-проверок и 6 прежнего DNS-стенда — PASS. Pcap покрывает короткую fault-matrix, не все пять минут. История, ограничения проверок и карта реализации: [контекст clean-vpn](docs/clean-vpn-context.md).

Следующий порядок работ:

1. Доработать оставшиеся ограничения [DNS wire contract](scripts/dns-wire.md): полный TCP/DoH размер DNS65535 с отдельным UDP cap и memory/framing budgets, EDNS version negotiation и HTTP cache-age/TTL. Обычные IN-типы, binary labels, extended EDNS RCODE и безопасный UDP TC уже реализованы и проверены; переключать системный DNS пока рано.
2. Затем отдельно согласовать opt-in интеграцию DNS с client/OS/LAN и IPv6/kill-switch: жизненный цикл, восстановление настроек и поведение при отказах. Без автоматического переключения работающей системы на этапе разработки.
