...
2. [Лабораторный журнал и SIGKILL recovery](scripts/dns-lifecycle.md) уже есть, включая [экспериментальный resolved backend](scripts/dns-resolved.md) с persistent per-setter intents и настоящим приватным D-Bus: `npm run test:dns-resolved-journal-real`. Далее — полный lifecycle adapter: смерть/перезапуск процесса, стабильный endpoint, readiness и сохранение guard; затем VM reboot/power-loss tests. Live DNS/firewall/TUN не меняются, resolved на клиенте автоматически не включается.
