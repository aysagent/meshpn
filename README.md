...
Для [`scripts/clean-vpn.js`](scripts/clean-vpn.js) на **Linux** нужен собранный модуль `native/tun_linux` (после `npm install` это делает postinstall, либо явно: `npm run build:tun-linux`; нужны python3, make, g++). По транспорту после преамбулы передаётся **один L3-кадр** — **IPv4 или IPv6** (см. шапку `clean-vpn.js`). `--split-default` отправляет **IPv4** default в туннель (`0.0.0.0/1` + `128.0.0.0/1`), **RFC1918** — на uplink. **`--split-default-v6`** (только вместе с `--split-default`) делает то же для **IPv6** (`::/1` + `8000::/1`), **ULA `fc00::/7`** и **link-local `fe80::/10`** — на uplink; нужен **`ip -6 route show default`**. На **exit** дополнительно включаются **`ip6tables` NAT** (MASQUERADE для адреса клиента **`fd10:99::2`**) и **`net.ipv6.conf.all.forwarding`**. Внешний IPv4: `curl -4 https://ifconfig.me`. Для `--type=tls` и IP в `--server` см. шапку скрипта. **Локальный ping:** **ICMPv4** к **`10.99.0.1`** (exit) / **`10.99.0.2`** (client) и **ICMPv6 Echo** к **`fd10:99::1`** / **`fd10:99::2`** (ULA на TUN) — ответы формируются в JS; проверка: `ping` / `ping6` на свой адрес на интерфейсе tun. Полный выход в интернет по IPv6 зависит от провайдера, PMTU и правил на exit.
...
Протокол тот же, что у `--type=websocket`: одно binary WS-сообщение = один IPv4- или IPv6-пакет.
