...
2. [Лабораторный журнал и SIGKILL recovery](scripts/dns-lifecycle.md) уже есть, включая [экспериментальный resolved backend](scripts/dns-resolved.md): `npm run test:dns-resolved-journal-real`. Добавлен [отдельный процесс DNS-adapter](scripts/dns-adapter-process.md), SIGKILL/restart на прежнем порту: `npm run test:dns-adapter-process-real`. Есть [offline boot-протокол и VM preflight](scripts/dns-boot.md), а также [изолированная QEMU-лаборатория](scripts/dns-vm-lab.md): `npm run dns:vm-lab -- --tools=... --kernel=... --resolved=...`. Она проверяет новые загрузки гостя и SIGKILL QEMU, но не физический power-loss и не systemd boot ordering. Следом — ранние boot-fault cases и systemd PID1 в госте. Live DNS/firewall/TUN не меняются, resolved на клиенте автоматически не включается.
