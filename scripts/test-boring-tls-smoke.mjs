...
test('helper: без log_ja3 на stderr нет ja3_md5', async (t) => {
  ...
  try {
    ...
    assert.ok(!stderr.includes('ja4_raw_r_alt_sni_alpn='), stderr);
  } finally {
    try {
      child.kill('SIGKILL');
    } catch {
...
test('helper: log_ja3=true — stderr содержит эталонный ja3_md5', async (t) => {
  ...
  try {
    ...
    assert.ok(
      stderr.includes(
        `ja4_raw_r_alt_sni_alpn=${j4.raw_r_alt_sni_alpn_in_segment}`,
      ),
      stderr,
    );
  } finally {
...
test('helper: profile_vs_wire extensions — строка диагностики при extension_types', async (t) => {
  if (!fs.existsSync(helper)) {
    t.skip();
    return;
  }
  ...
  try {
    sendConfigFrame(child.stdin, {
      ...
      client_hello_profile: {
        ...
        ec_point_formats: [0],
        permute_extensions: false,
        /** Типа не будет в реальном ClientHello — multiset diff непустой. */
        extension_types: [60138],
      },
    });
    ...
    await raceMs(
      new Promise((resolve, reject) => {
        ...
        const tick = () => {
          if (stderr.includes('profile_vs_wire extensions:')) {
            resolve(undefined);
            return;
          }
          if (Date.now() > deadline) {
            reject(new Error('нет profile_vs_wire extensions на stderr'));
            return;
          }
          setImmediate(tick);
        };
