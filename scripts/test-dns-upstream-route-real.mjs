/** Opt-in actual TCP/TLS tests on private-loopback public-address aliases. No external network. */
...
for (const modeTag of ['transparent-tls', 'combo-tls']) for (const family of [4, 6]) {
  test(`${modeTag} pinned IPv${family}: refusal, verified DoH, wrong CA, reset, exhaustion, restart`, { timeout: 20000 }, async (t) => {
    const directory = await mkdtemp(join(tmpdir(), 'meshpn-dns-route-real-'));
    ...
    const source = `
      ...
      try {
        const result = await runPinnedDnsRouteLab(process.env.MESHPN_ROUTE_DIR, '${modeTag}', ${family});
        process.stdout.write('DNS_ROUTE_RESULT ' + JSON.stringify(result) + '\\n');
      } catch (error) { process.stderr.write('DNS_ROUTE_FAILED ' + (error.code ?? error.name)); process.exitCode = 1; }
    `;
    const result = await runCommand('unshare', [...namespaceArgs, 'sh', '-eu', '-c', 'ulimit -c 0; ip link set lo up; exec "$@"',
      'dns-route', process.execPath, '--input-type=module', '-e', source], { timeoutMs: 15000,
    ...
    const lines = result.stdout.trim().split('\n'); assert.equal(lines.length, 1); assert.ok(lines[0].startsWith('DNS_ROUTE_RESULT '));
    const report = JSON.parse(lines[0].slice(17));
    assert.equal(report.status, 'passed'); assert.equal(report.family, family); assert.equal(report.modeTag, modeTag);
    assert.equal(report.tcpAttempts, 10); assert.equal(report.dnsCalls, 0); assert.equal(report.resolverBodies, 3);
