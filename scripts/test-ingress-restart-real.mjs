...
test('continuous IPv4/IPv6 ingress probes cannot escape through restart windows', { timeout: 60000 }, async () => {
  const result = await runCommand('unshare', [...namespaceArgs.map((a) => a === '--map-current-user' ? '--map-root-user' : a),
    process.execPath, '--input-type=module', '-e', `
      ...
      console.log(JSON.stringify(await runIngressRoutingLab({restart:true})));
    `], { timeoutMs: 55000, env: { ...cleanEnvironment(process.env),
  ...
  assert.equal(report.probePackets.uplink, 0); assert.ok(report.probePackets.tunnel > 0);
