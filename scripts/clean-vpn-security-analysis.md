...
Краткие записи по уже закрытым пунктам Phase 1 (M-1, M-2, M-4, M-5, H-4) и Phase 2 (H-1+H-2, C-2). Каждая запись: суть исходной проблемы, применённый фикс и остаточный риск.

### Fixed M-1 — TLS cover-page без rate-limit
...
### Fixed M-4 — combo-tls peek timeout / медленный DoS
