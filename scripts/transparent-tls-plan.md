...
- **Перехват приложений** на шлюзе: **iptables** `nat OUTPUT` для **локальных** процессов (tcp/443) и при **`--client-lan-subnet`** дополнительно **`nat PREROUTING`** для проброса с LAN; **`SO_ORIGINAL_DST`** в коде клиента (**`native/tun_linux`**). Перенаправление на **`127.0.0.1`:** для проброшенных пакетов обычно нужен **`net.ipv4.conf.all.route_localnet=1`** на время работы (скрипт выставляет только если перехват LAN активен). Для JA3-сравнений используйте **HTTPS**, не HTTP (порт **80 не перехватывается**).
