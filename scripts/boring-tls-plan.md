...
| `emit_sni` | bool | в IPC helper (ставит `clean-vpn`): **`false`** — не вызывать `SSL_set_tlsext_host_name`, в ClientHello **нет** расширения server_name (тип 0) → в JA4_a маркер **`i`** вместо **`d`**. Источник: только **`clienthello_emit_sni`** в файле профиля; если ключа **нет** — **`true`** (без SNI CDN часто отдают дефолтный сертификат → **`CERTIFICATE_VERIFY_FAILED`** при **`verify_host`**). Явное **`clienthello_emit_sni: false`** — сознательный компромисс ради JA4 как у клиента без SNI |

**Файл профиля (disk):** опционально **`clienthello_emit_sni`** (boolean): сохраняется при экспорте из ja3-snif — **`true`**, если в захвате было расширение server_name (`extension_types` содержит **`0`**); **`false`** — если в захвате SNI не было. Старые JSON без этого ключа ведут себя как **`emit_sni: true`** (TLS к именованным хостам не ломается).

**JA4 (FoxIO):** один отпечаток `ja4_a_ja4_b_ja4_c`; алгоритм совпадает с [`tls-clienthello-ja4.mjs`](./lib/tls-clienthello-ja4.mjs) и `ComputeJa4FromClientHelloBody` в helper. Две цифры в середине **JA4_a** после `i`/`d` — число cipher и число **типов расширений** (без GREASE): расхождение вроде **15 vs 11** обычно означает, что **BoringSSL шлёт меньше расширений**, чем браузер в ja3-snif; **`emit_sni`** устраняет только вклад **SNI** (`d` vs `i` и минус одно расширение). **`JA4_c`** зависит от набора типов расширений (без SNI/ALPN в части сортировки) и от списков **signature algorithms** в расширениях **13** и **50**, объединённых **в порядке обхода расширений на проводе**. Поэтому полное совпадение JA4 с браузерным снимком не гарантируется только профилем cipher/groups — стек BoringSSL может отличаться по расширениям и содержимому 13/50; при **`permute_extensions`** относительный порядок ext **13** и **50** на wire может менять **`JA4_c`**.
