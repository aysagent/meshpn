...
| `emit_sni` | bool | в IPC helper (ставит `clean-vpn`): **`false`** — не вызывать `SSL_set_tlsext_host_name`, в ClientHello **нет** расширения server_name (тип 0) → в JA4_a маркер **`i`** вместо **`d`**. По умолчанию из файла профиля: **`clienthello_emit_sni`**, либо выводится из **`extension_types`** (есть ли **`0`**). Нужно совпасть с захватом ja3-snif (HTTPS на литерал IP часто без SNI); проверка сертификата по **`verify_host`** при этом сохраняется |

**Файл профиля (disk):** опционально **`clienthello_emit_sni`** (boolean): сохраняется при экспорте из ja3-snif — **`true`**, если в захвате было расширение server_name (`extension_types` содержит **`0`**).
