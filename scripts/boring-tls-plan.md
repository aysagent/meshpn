...
- **JA4 raw / типы расширений:** в строках `ja4_raw_*` каждый тип — **4 hex-цифры (uint16 big-endian)**. Расширение **Extended Master Secret** — IANA **23** (decimal), на проводе это **`0017`**, не **`0023`**. Последовательность **`0023`** в JA4 — это тип **0x0023 = 35** (**session_ticket**, RFC 5077 для TLS 1.2).
...
- **Session ticket (ext 35 / в JA4 hex `0023`, TLS 1.2):** BoringSSL по умолчанию всегда добавляет объявление **session_ticket**, даже с пустым телом. Если в **`extension_types`** **нет** типа **35**, helper выставляет **`SSL_OP_NO_TICKET`** и расширение не уходит на провод (паритет с захватами без ticket). Если **35** указан — флаг снимается. При отсутствии или пустом **`extension_types`** ticket тоже подавляется. На stderr — **`session_ticket extension ...`**.
