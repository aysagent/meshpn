...
**Сверка при расхождении с сайтом или другим инструментом:** на одном и том же PCAP (или hex одного ClientHello) сравнивайте **`ja4_raw_r`** helper/clean-vpn со строкой «raw» калькулятора (FoxIO JA4.md — это JA4_r, не JA4_ro). Если при идентичных байтах fingerprint всё равно расходится, проверьте тот же файл в **Wireshark / `tshark`** с полем JA4 той же версии движка FoxIO (разные версии дисектора могут давать разный JA4 до выравнивания спеки).
