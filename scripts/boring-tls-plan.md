...
| `log_ja3` | bool | если `true`, после исходящего ClientHello на stderr: `ja3_md5=`, `ja3_sorted_md5=`, **`ja4=`** (FoxIO JA4.md), **`ja4_alt_sni_alpn_in_j4c=`** (иной `JA4_c`, см. ниже), **`ja4_raw_o`**, **`ja4_raw_r`** (JA4.md JA4_r: средний сегмент без 0000/0010), **`ja4_raw_r_alt_sni_alpn=`** (тот же формат строки, но средний сегмент включает 0000/0010 — близко к ja3.zone, не JA4.md); при `ja3_verbose` — строки JA3 до MD5, компоненты JA4 (`ja4_a`/`ja4_b`/`ja4_c`, `ja4_c_alt_sni_alpn_in_hash`) и `hex_preview` handshake |
...
**Сверка при расхождении с сайтом или другим инструментом:** строка **`ja4_raw_r`** — именно JA4.md JA4_r (в среднем сегменте **нет** 0000/0010). У ja3.zone и части калькуляторов «raw» ближе к **`ja4_raw_r_alt_sni_alpn`** (те же отсортированные cipher и те же sig по проводу, но средний сегмент **включает** 0000 и 0010 — это уже не JA4.md). Сравнение возможно только по **одним и тем же байтам** ClientHello: у сайта **`1517`** в префиксе при ваших **`1516`** уже означает **другой** захват (например, у них есть расширение **`0029`**, у вас в списке его нет). Если при идентичных байтах fingerprint всё расходится — см. **Wireshark / `tshark`** и версию JA4-движка.
