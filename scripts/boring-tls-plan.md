...
- **Extended Master Secret (ext 23 / `0023` в JA4):** BoringSSL для ClientHello с минимальной версией **TLS 1.2** по умолчанию добавляет EMS. Чтобы **совпадать с профилем**, helper подавляет EMS через патч **`SSL_CTX_set_meshvpn_suppress_extended_master_secret_clienthello`**, пока в **`extension_types`** профиля **нет** типа **23**; если **23** указан — EMS уходит на провод как в захвате. Если **`extension_types` отсутствует или пустой**, EMS тоже **подавляется** (минимальные профили без списка расширений не должны получать «лишний» `0023`). На stderr: **`extended_master_secret ClientHello suppressed`** или **`enabled (type 23 in profile)`**.
