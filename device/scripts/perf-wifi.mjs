...
export function cleanWifiDiagnostics(wifi) {
  const r=wifi?.radio;
  return {
    tx:{...pick(wifi?.tx,['available','buffer_type','static_rx_buffer_count','static_buffer_count','dynamic_buffer_count','cache_buffer_count','amsdu_enabled',
...
export function validateWifiTxExperiment(tx) {
  ...
  const knownBufferPair=(tx.static_rx_buffer_count===16&&tx.static_buffer_count===24)||
    (tx.static_rx_buffer_count===12&&tx.static_buffer_count===28);
  if(tx.buffer_type!=='static'||!knownBufferPair||tx.cache_buffer_count!==128||tx.amsdu_enabled!==false||
    ...
    return `AP test requires Wi-Fi static RX/TX=16/24 (control) or 12/28 (experiment), cache=128, A-MSDU=false, A-MPDU TX/RX=true, BA=12/24, IRAM Wi-Fi/extra/RX/lwIP=true/false/true/true; got type=${tx.buffer_type??'n/a'}, static RX/TX=${tx.static_rx_buffer_count??'n/a'}/${tx.static_buffer_count??'n/a'}, cache=${tx.cache_buffer_count??'n/a'}, A-MSDU=${tx.amsdu_enabled??'n/a'}, A-MPDU TX/RX=${tx.ampdu_enabled??'n/a'}/${tx.ampdu_rx_enabled??'n/a'}, BA=${tx.ampdu_ba_window??'n/a'}/${tx.ampdu_rx_ba_window??'n/a'}, IRAM=${tx.iram_opt_enabled??'n/a'}/${tx.extra_iram_opt_enabled??'n/a'}/${tx.rx_iram_opt_enabled??'n/a'}/${tx.lwip_iram_opt_enabled??'n/a'}. Flash matching firmware first.`;
  return null;
}
...
export function wifiMarkdown(records) {
  ...
  if(config)lines.splice(3,0,`Wi-Fi buffers: type=${config.buffer_type??'n/a'}, static RX/TX=${n(config.static_rx_buffer_count)}/${n(config.static_buffer_count)}, dynamic TX=${n(config.dynamic_buffer_count)}, cache TX=${n(config.cache_buffer_count)}, A-MSDU=${typeof config.amsdu_enabled==='boolean'?config.amsdu_enabled:'n/a'}, A-MPDU TX/RX=${typeof config.ampdu_enabled==='boolean'?config.ampdu_enabled:'n/a'}/${typeof config.ampdu_rx_enabled==='boolean'?config.ampdu_rx_enabled:'n/a'}, BA=${n(config.ampdu_ba_window)}/${n(config.ampdu_rx_ba_window)}, IRAM Wi-Fi/extra/RX/lwIP=${['iram_opt_enabled','extra_iram_opt_enabled','rx_iram_opt_enabled','lwip_iram_opt_enabled'].map(k=>typeof config[k]==='boolean'?config[k]:'n/a').join('/')}.`,'');
