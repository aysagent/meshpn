...
export function cleanWifiDiagnostics(wifi) {
  ...
  return {
    tx:{...pick(wifi?.tx,['available','buffer_type','static_buffer_count','dynamic_buffer_count','cache_buffer_count','amsdu_enabled',
      'ampdu_enabled','ampdu_ba_window','ampdu_rx_enabled','ampdu_rx_ba_window']),...Object.fromEntries(['sta','ap'].map(k=>
...
export function wifiMarkdown(records) {
  ...
  if(config)lines.splice(3,0,`TX buffers: type=${config.buffer_type??'n/a'}, static=${n(config.static_buffer_count)}, dynamic=${n(config.dynamic_buffer_count)}, cache=${n(config.cache_buffer_count)}, A-MSDU=${typeof config.amsdu_enabled==='boolean'?config.amsdu_enabled:'n/a'}, A-MPDU TX/RX=${typeof config.ampdu_enabled==='boolean'?config.ampdu_enabled:'n/a'}/${typeof config.ampdu_rx_enabled==='boolean'?config.ampdu_rx_enabled:'n/a'}, BA=${n(config.ampdu_ba_window)}/${n(config.ampdu_rx_ba_window)}.`,'');
