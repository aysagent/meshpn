...
test('Wi-Fi diagnostics preserve separate counter deltas, unknown/reset and sanitized peers',()=>{
  const state=n=>({uptime_sec:n,wifi:{tx:{available:true,buffer_type:'static',static_buffer_count:24,cache_buffer_count:128,
  ...
  assert.ok(text.includes('static=24'));assert.ok(text.includes('cache=128'));
