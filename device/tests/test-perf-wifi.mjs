...
test('Wi-Fi diagnostics preserve separate counter deltas, unknown/reset and sanitized peers',()=>{
  ...
  assert.equal(after.wifi.tx.extra_iram_opt_enabled,false);assert.equal(after.wifi.tx.lwip_iram_opt_enabled,true);
  ...
  assert.match(validateWifiTxExperiment({...after.wifi.tx,extra_iram_opt_enabled:true}),/IRAM=true\/true\/true\/true/);
