...
test('Wi-Fi diagnostics preserve separate counter deltas, unknown/reset and sanitized peers',()=>{
  const state=n=>({uptime_sec:n,wifi:{tx:{available:true,buffer_type:'static',static_buffer_count:24,cache_buffer_count:128,amsdu_enabled:false,
    ampdu_enabled:true,ampdu_ba_window:24,ampdu_rx_enabled:true,ampdu_rx_ba_window:24,
  ...
  assert.equal(after.wifi.tx.ampdu_ba_window,24);assert.equal(after.wifi.tx.ampdu_rx_ba_window,24);
  ...
  assert.ok(text.includes('A-MPDU TX/RX=true/true'));assert.ok(text.includes('BA=24/24'));
  ...
  assert.match(validateWifiTxExperiment({...after.wifi.tx,ampdu_ba_window:12}),/BA=12\/24/);
