...
Админка/mDNS по-прежнему только USB; AP разрешён DNS на собственном gateway, со STA закрыты оба DNS.
...
2. Проверить HTTPS, USB/AP-доступ к админке, USB-only mDNS `meshpn.local`, fallback `meshpn.home.arpa`/IP, импорт собственного сертификата и недоступность management/DNS со стороны upstream STA.
...
Kconfig теперь задаёт только начальное значение; чекбокс в админке сохраняет выбор в NVS и применяет после перезагрузки без перепрошивки. В HTTPS-режиме HTTP служит только redirect. HTTP boot не инициализирует сертификат; certificate API запрещены по HTTP. Авторизация и изоляция от WiFi сохраняются, но передача credentials по HTTP не зашифрована.
