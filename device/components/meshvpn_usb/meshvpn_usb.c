...
static esp_err_t meshvpn_usb_transmit(void *h, void *buffer, size_t len)
{
    ...
    /* Retry without vTaskDelay — busy (ESP_FAIL) returns fast; timeout only if USB
     * task stalls. Phase 7a: shorter timeout, more yield-only retries. */
    for (int attempt = 0; attempt < 128; attempt++) {
        err = tinyusb_net_send_sync(buffer, (uint16_t)len, NULL, pdMS_TO_TICKS(5));
        if (err == ESP_OK) {
            if (attempt > 0) {
                s_stats.tx_retried++;
            }
...
esp_err_t meshvpn_usb_attach_netif(esp_netif_t *netif)
{
    ...
    ESP_LOGI(TAG, "USB sync TX installed (128x5ms retry)");
