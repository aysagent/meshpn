...
static void meshvpn_usb_tx_worker(void *arg)
{
    ...
    while (true) {
        ...
        meshvpn_usb_send_with_retry(job.buf, job.len);
        free(job.buf);
    }
}
...
static esp_err_t meshvpn_usb_tx_async_start(void)
{
    ...
    BaseType_t ok = xTaskCreatePinnedToCore(meshvpn_usb_tx_worker, "usb_tx", 4096, NULL, 8,
                                            NULL, 1);
    if (ok != pdPASS) {
    ...
    ESP_LOGI(TAG, "USB async TX worker (queue %d, %dx%dms retry on CPU1)",
             USB_TX_QUEUE_DEPTH, USB_TX_RETRY_MAX, USB_TX_RETRY_MS);
    return ESP_OK;
}
