...
Для transparent/combo HTTPS-перехват в этом режиме только через PREROUTING выбранного интерфейса, listener на собственном TUN IPv4; прежний OUTPUT-перехват не включается. Это не исправляет отсутствие защиты сырого TUN у standalone transparent. Без restart-safe штатный stop возвращает прежний forwarding; после fatal/SIGKILL guard/rules сохраняются. Новые запуски используют `ingress-journal.mjs`: fsync/rename, write-ahead intent, namespace flock, исходные rp_filter и link identity. `clean-vpn-recover.mjs --from-tun=wg0` выполняет dry-run, с `--apply` возвращает прежний forwarding. Recovery повторяемый, только same-boot; отказывается при конфликтующем/чужом состоянии. Legacy-состояние без журнала не присваивается. Подробности, тесты и ограничения — [руководство ingress-шлюза](../scripts/clean-vpn-from-tun.md). Supervisor/systemd, сохранение сессий при restart, boot kill-switch, DNS-прокси LAN и реальный WireGuard-пилот этим изменением не покрыты.

Важно при дальнейшей разработке ingress: schema 1 журнала хранит курсор детерминированного плана, который генерируется тем же installer, а не исполняемые команды из JSON. При изменении порядка/семантики операций нужна новая schema или явная миграция. Пользователь должен завершать recovery до обновления версии (без restart-safe достаточно штатного stop); прямой forwarding при этом возвращается, поэтому peers при необходимости заранее приостанавливаются. Команды изменения состояния наследуют lock fd, чтобы SIGKILL Node не разблокировал recovery раньше завершения уже запущенного iptables/ip/sysctl.

Дополнение restart-safe: `--from-tun-restart-safe` паркует клиент под IPv4/IPv6 FORWARD DROP и IPv4 INPUT DNAT DROP; следующий запуск очищает/ставит маршруты под тем же namespace-lock. Schema 2 добавляет hold ID/interface/intent cursor; schema 1 по-прежнему читается и может быть явно принята под guard. Базовый план schema 1 не переставлен. Guard снимается после установки transport handlers/listeners и проверки полного плана; lazy handshake может начаться только после первого пакета. Временный DROP включает пересылку LAN, но не host OUTPUT/другие входы/обычные локальные сервисы. Полный возврат baseline — только explicit recovery --apply. Нет сохранения сессий, автоматического запуска процесса, supervisor или boot kill-switch. DNS v1 по-прежнему требует подготовки одной live-конфигурации и 24-часового клиентского пилота.

Попутные исправления общего TUN-моста при restart: eager/lazy запуск используют один `ensureWire`, чтобы первый пакет не создавал параллельную сессию; новая сессия получает чистый StreamFramer без недочитанного фрагмента старой; запоздалый write callback старого endpoint не закрывает новый. Пять регрессионных тестов в `test-tun-bridge-startup.mjs` исполняют реальные функции CLI с тестовыми IO и входят в full acceptance. Это не перенос mesh-реализации и не изменение браузерного профиля.

Итог restart-safe, 2026-09-25: namespace routing 24 проверки; restart 26 проверок; recovery 24 + 9 crash points; NIC-less VM — tls/boring-tls/transparent-tls/combo-tls по 25 проверок, uplink probes = 0, точный baseline restore. 188 файлов VM совпали по SHA256. Full acceptance: 1079 Node + 14 browser PASS; один промежуточный прогон имел прежний flaky descendant-cleanup test. Ранние VM попытки с 4-секундным TLS deadline не прошли; финальный ограниченный бюджет TLS в TCG — 12 секунд, пробный трафик остаётся непрерывным. Локальные итоговые отчёты: `/var/tmp/meshpn-ingress-vm-cdGwsU/report.json`, `/var/tmp/meshpn-acceptance-yHgUwd/report.json`. Дальше: выбрать live DNS-клиента/владельца, подготовить реальные конфигурацию/units и откат (не устанавливать VM fixtures), проверить их в VM; с отдельным разрешением выполнить ограниченный 24-часовой пилот и закрыть DNS v1 по `scripts/dns-v1.md`. Затем — настоящий WireGuard ingress/MTU/нагрузка. Ни live DNS, ни live wg0 этим этапом не изменялись.

Сбор клиентской диагностики упрощён по просьбе пользователя: вместо списка ручных команд запускать `node scripts/dns-diagnostic.mjs --probe` и передавать весь stdout между BEGIN/END. Без флага — без DNS-запросов; с флагом — фиксированный example.com через текущий resolver (возможен прямой путь и обычное on-demand поведение NSS). Настройки не меняются; D-Bus getters направляются уникальному владельцу с auto-start=no, явных команд запуска служб нет. Отчёт включает адреса/домены, фильтрованные DNS-поля конфигов, маршруты и listeners, но не argv, журналы, `.env`, `.netdev` или приватные ключи. Ограничения и ошибки включаются в отчёт; это не доказательство отсутствия DNS-утечек или владения настройками. Подробности: `scripts/dns-inspect.md`. По результату продолжать подготовку DNS v1, не считать её установленной самим запуском диагностики. Проверено: 10 целевых тестов, 18 прежних dns-inspect; настоящий inspection-only запуск выдал отчёт, resolv.conf не изменился. Node acceptance — 1089 PASS (`/var/tmp/meshpn-acceptance-d3X0ob/report.json`), браузерная матрица в этом изменении не запускалась. Первый Node-прогон упал на прежнем UDP/TCP bind rollback test (`/var/tmp/meshpn-acceptance-luzZjM/report.json`); отдельно 46 тестов этого файла прошли, неудачный отчёт сохранён.
