...
План по обсуждению 2026-09-23, **не реализован**. Текущий пакет меняет только
destination admission transparent exit; DNS клиента/системы и firewall не менялись.
...
Прямые cover-запросы **не включены**. Следующий порядок: безопасный IP failover
exit → DNS privacy/explicit-loopback стенд → отдельное решение о системной
интеграции и только затем измеримый cover DNS эксперимент, если останется смысл.
